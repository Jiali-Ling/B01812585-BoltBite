                    ...
                    @forelse ($menuItems as $item)
                        <div class="bg-white border border-gray-200 rounded-lg overflow-hidden hover:shadow-lg transition-shadow">
                            <div class="relative w-full" style="height: 180px; background-color: #f3f4f6; overflow: hidden;">
                                @if ($item->image_url)
                                    <img src="{{ $item->image_url }}" alt="{{ $item->name }}" class="w-full h-full" style="object-fit: cover; height: 180px; image-rendering: -webkit-optimize-contrast; image-rendering: crisp-edges;" loading="lazy" decoding="async">
                                @else
            ...
            <div class="bg-white shadow sm:rounded-lg p-6">
                ...
                <form action="{{ route('merchant.menu-items.store', $restaurant) }}" method="POST" enctype="multipart/form-data" class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-gray-500 mt-1">Select a file to upload (PNG, JPG, GIF up to 10MB)</p>
                        @error('image')
                            <p class="text-red-500 text-xs mt-1">{{ $message }}</p>
                        @enderror
                    </div>
